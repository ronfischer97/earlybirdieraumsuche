"""
EARLYBIRDIE — Crawler v4 (ScraperAPI)
Umgeht 403-Blockierungen via echte Heim-IPs
"""
...
# ScraperAPI Key aus Railway-Umgebungsvariable
...
def scraper_fetch(url: str) -> Optional[BeautifulSoup]:
    """Holt eine URL via ScraperAPI — umgeht 403-Blockierungen."""
    if not SCRAPER_KEY:
        log.error("SCRAPER_API_KEY nicht gesetzt!")
        return None
    api_url = "https://api.scraperapi.com/"
    params  = {
        "api_key":        SCRAPER_KEY,
        "url":            url,
        "country_code":   "ch",   # Schweizer IP bevorzugen
        "render":         "false", # Kein JS-Rendering (spart Credits)
    }
    try:
        r = requests.get(api_url, params=params, timeout=60)
        log.info("ScraperAPI → %s: HTTP %d", url[:60], r.status_code)
        if r.status_code == 200:
            return BeautifulSoup(r.text, "html.parser")
        else:
            log.warning("ScraperAPI Fehler %d für %s", r.status_code, url[:60])
    except Exception as e:
        log.warning("ScraperAPI Exception: %s", e)
    return None


# ─────────────────────────────────────────────
# PORTAL 1: IMMOSCOUT24
# ─────────────────────────────────────────────
def scrape_immoscout(city: str, lat: float, lon: float) -> list[dict]:
    ...
    try:
            ...
            try:
                ...
                for item in items:
                    ...
                    results.append({
                        ...
                        "image_url": str(img), "listing_url": url_l,
        ...
        if not results:
            for card in soup.find_all("article")[:20]:
                ...
                results.append({
                    ...
                    "portal": "ImmoScout24",
                    "title": text[:80], "city": city, "address": city,
                    "area_m2": area, "price_chf": price,
    ...
    try:
        ...
        if not results:
            for card in soup.find_all(["article","div"],
                                       class_=re.compile(r"listing|result|item", re.I))[:20]:
                text = card.get_text(" ", strip=True)
                if len(text) < 20:
                    continue
                a    = card.find("a", href=True)
...
# ─────────────────────────────────────────────
# PORTAL 3: FLATFOX (JSON-API via ScraperAPI)
# ─────────────────────────────────────────────
def scrape_flatfox(city: str, lat: float, lon: float) -> list[dict]:
    ...
    url = (f"https://flatfox.ch/api/v1/listing/"
           ...
           f"&floor_space_from={MIN_AREA}&floor_space_to={MAX_AREA}")
    if not SCRAPER_KEY:
        return []
    try:
        api_url = "https://api.scraperapi.com/"
        params  = {"api_key": SCRAPER_KEY, "url": url, "country_code": "ch"}
        r = requests.get(api_url, params=params, timeout=60)
        if r.status_code == 200:
            data = r.json()
            for item in data.get("results", [])[:30]:
                ...
                results.append({
                    ...
                    "city": city, "address": item.get("street") or city,
...
def run_crawl():
    ...
    log.info("═══ Earlybirdie Crawl v4 gestartet · %s ═══", run_id)
    init_db()

    if not SCRAPER_KEY:
        log.error("SCRAPER_API_KEY fehlt! Crawler abgebrochen.")
        return

    total_found = total_inserted = 0
    scrapers = [scrape_immoscout, scrape_homegate, scrape_flatfox]
    ...
    try:
        conn = get_connection()
        cur  = conn.cursor()
        cur.execute("INSERT INTO crawl_log VALUES (%s,%s,%s,%s,%s)",
            (run_id, str(total_found), str(total_inserted),
             started, datetime.now(timezone.utc).isoformat()))
        conn.commit()
        conn.close()
    except Exception as e:
        log.warning("crawl_log Fehler: %s", e)
